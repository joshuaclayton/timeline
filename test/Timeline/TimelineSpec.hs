...
spec = parallel $
    describe "parseGraphs" $ do
        ...
        it "parses scatter plot charts" $ do
            ...
            result `shouldBe` Graphs [Graph (Just "Awesome") $ ScatterPlotGraph [("Red", 5, 1), ("Red", 5, 3), ("Blue", 1, 0), ("Blue", 2, 2), ("Green", 1.75, 4)]]

        it "parses multiple charts" $ do
            let (Right result) = parseGraphs "bar: 1,2,3\nline: 1,2,3\nbar: 5,5,5\nbar: -5,-5,-5"
            result `shouldBe` Graphs [Graph Nothing $ BarGraph [1, 2, 3], Graph Nothing $ LineGraph [1, 2, 3], Graph Nothing $ BarGraph [5, 5, 5], Graph Nothing $ BarGraph [-5, -5, -5]]

        it "parses stacked charts in normal order" $ do
            let (Right result) = parseGraphs "stacked-bar: [1,2,3],[4,5,6]\nline: 1,2,3"
            result `shouldBe` Graphs [Graph Nothing $ StackedBarGraph [[1, 2, 3], [4, 5, 6]], Graph Nothing $ LineGraph [1, 2, 3]]

        it "parses stacked charts in reverse order" $ do
            let (Right result) = parseGraphs "line: 1,2,3\nstacked-bar: [1,2,3],[4,5,6]"
            result `shouldBe` Graphs [Graph Nothing $ LineGraph [1, 2, 3], Graph Nothing $ StackedBarGraph [[1, 2, 3], [4, 5, 6]]]

        it "parses box plots" $ do
            let (Right result) = parseGraphs "box-plot: [1,2,3,10,20], [3,4,7,10,20], [4,5,6,20,25]"
            result `shouldBe` Graphs [Graph Nothing $ BoxGraph [Just $ BoxAndWhisker 1 2 3 10 20, Just $ BoxAndWhisker 3 4 7 10 20, Just $ BoxAndWhisker 4 5 6 20 25]]
        ...
        it "supports sma with different values" $ do
            ...
            result `shouldBe` Graphs [Graph Nothing $ LineGraph [0,2,4,6,8], Graph (Just "SMA(1)") $ LineGraph [0, 2, 4, 6, 8]]

            let (Right result) = parseGraphs "line: 0,2,4,6,8 +sma(2)"
            result `shouldBe` Graphs [Graph Nothing $ LineGraph [0,2,4,6,8], Graph (Just "SMA(2)") $ LineGraph [0, 1, 3, 5, 7]]

            let (Right result) = parseGraphs "line: 0,2,4,6,8 +sma(3)"
            result `shouldBe` Graphs [Graph Nothing $ LineGraph [0,2,4,6,8], Graph (Just "SMA(3)") $ LineGraph [0, 1, 2, 4, 6]]

            let (Right result) = parseGraphs "line: 0,2,4,6,8 +sma(4)"
            result `shouldBe` Graphs [Graph Nothing $ LineGraph [0,2,4,6,8], Graph (Just "SMA(4)") $ LineGraph [0, 1, 2, 3, 5]]
        ...
        it "supports multiple additionals" $ do
            ...
            result `shouldBe` Graphs [Graph Nothing $ LineGraph [0, 2, 4, 6, 8], Graph (Just "SMA(1)") $ LineGraph [0, 2, 4, 6, 8], Graph (Just "SMA(4)") $ LineGraph [0, 1, 2, 3, 5]]
